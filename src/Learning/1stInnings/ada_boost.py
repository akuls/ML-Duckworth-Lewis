...
f_redirect = open('ada_boost.txt', 'w')
...
def regress(trainX, trainY, testX, testY, clf):
    ...
    with open('ada_boost.pickle', 'wb') as f:
        pickle.dump(clf, f)
...
def pipeline(X_tr, Y_tr):
    ...
    param_grid = {"n_estimators": [3000], "max_features": ["sqrt"]}
    clf = hyperparameter_selection(X_train, y_train, method=ensemble.AdaBoostRegressor(random_state=44, n_estimators=1000, learning_rate=0.001))
